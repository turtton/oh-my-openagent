...
- **003-noreply-true**: Forces all background task completion notifications to use `noReply: true`. Without this patch, when all background tasks complete, a notification with `noReply: false` triggers an unnecessary extra agent turn — even when the agent already received results via the in-turn `block=true` call (patch 001). This patch suppresses the notification-triggered turn, avoiding the extra premium request that turn would consume.
